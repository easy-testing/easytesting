...
// Conta quantos nós cuja chave é k existem na árvore em O(log n).
int TreeCount(Node* x, Type k) {
  int count = 0;
  while (x != NULL) {
    if (k < x->key) {
      x = x->left;
