...
float DeterminanteMatriz(float A[][MAX]) {
  ...
  for (int i = 0;i < 3;++i) {
    pos = neg = 1;
    for (int j = 0;j < 3;++j) {
      // Multiplica os elementos da parte positiva do det.
      pos *= A[ j ] [ (i+j)%3 ];
      // Multiplica os elementos da parte negativa do det.
      neg *= A[ j ] [ 2 - (i+j)%3 ];
    }
    det += pos - neg;
  }
...
bool VerificaTriangularInferior(int n, float A[][MAX]) {
  for (int i = 0;i < n;++i)
    for (int j = i+1;j < n;++j)
      // Verifica se algum elemento da parte diagonal superior nao eh nula.
      if (!cmpfloat(A[i][j], 0.))
        return false;
  return true;
}
...
bool VerificaTriangularSuperior(int n, float A[][MAX]) {
  for (int i = 0;i < n;++i)
    for (int j = 0;j < i;++j)
      // Verifica se algum elemento da parte diagonal inferior nao eh nula.
      if (!cmpfloat(A[i][j], 0))
        return false;
  return true;
}
