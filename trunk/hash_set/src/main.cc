...
//
// Questão 1.
// Implemente o TAD hash_set<Type> em hash_set.h utilizando
// tabelas de dispersão.
//
// Questão 2.
// Escreva uma função "void Imprimir(hash_set<string>& s)" que recebe
// como parâmetro um conjunto s e imprime os elementos de s na tela.
// DICA: Utilize a função hash_set<Type>::ToList(list<Type>* l).
//
// Questão 3.
// Escreva uma função
// "void Intersecao(hash_set<string>& a, hash_set<string>& b,
// hash_set<string>* inter)" que recebe dois conjuntos 'a' e 'b'
...
//
// DICA: crie um conjunto "hash_set<string> candidatos" que contém inicialmente o
// nome de todos os fornecedores e um vetor "hash_set<string> pecas[100]" com o
...
void Imprimir(hash_set<string>& s) {
  list<string> l;
  s.ToList(&l);
  cout << "{";
...
void Intersecao(hash_set<string>& a, hash_set<string>& b, hash_set<string>* inter) {
...
int main() {
  hash_set<string> pecas[100];  // pecas[i] contém os fornecedores da peça i + 1.
  hash_set<string> candidatos;
  ...
  while (!candidatos.empty() && num_peca != 0) {
    ...
    if (num_peca >= 1 && num_peca <= 100) {
      hash_set<string> inter;
      Intersecao(pecas[num_peca - 1], candidatos, &inter);
      candidatos = inter;
      cout << "\n\nCandidatos: ";
      hash_set<string> aux = candidatos;
      Imprimir(aux);
    }
  }
  return 0;
